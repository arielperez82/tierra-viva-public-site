...
*Must be done first. All tools runnable via pnpm scripts; pre-commit runs type-check, lint, format, and markdown lint. Lighthouse runs as script (and optionally CI) for a11y/SEO/performance.*

### TV-0.1 — TypeScript type-checking (TSC / astro check)
...
### TV-0.2 — Husky and lint-staged pre-commit hooks
...
**Acceptance criteria:**
- [ ] `husky` and `lint-staged` installed; `pnpm prepare` runs `husky`.
- [ ] `.husky/pre-commit` runs `npx lint-staged` (or `pnpm pre-commit`).
- [ ] `lint-staged.config.js` (or equivalent): when any `*.{ts,tsx,js,jsx,astro}` is staged, run full-project type-check (e.g. `pnpm check`); run ESLint fix, Prettier fix, MarkdownLint fix on staged files.
- [ ] Pre-commit blocks commit if any check fails.
...
### TV-1.1 — Initialize Astro project with static output and TypeScript
